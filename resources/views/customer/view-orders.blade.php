			...
			<div class="col-md-12">
				{!! Form::open(['url'=>'home/orders','method'=>'get','class'=>'form-inline']) !!}
					...
					<div class="form-group {!! $errors->has('status') ? 'has-error' : '' !!}">
						{!! Form::select('status', [''=>'']+App\Order::statusList(), isset($status) ? $status : null, ['class'=>'form-control']) !!}
						{!! $errors->first('status','<p class="help-block">:message</p>') !!}
					</div>
					{!! Form::submit('Cari', ['btn btn-primary']) !!}
				{!! Form::close() !!}
						...
						@forelse($orders as $order)
							<tr>
								...
								<td>
									...
									Transfer ke : {{ config('bank-account')[$order->bank]['bank'] }} a.n. {{ config('bank-account')[$order->bank]['name'] }} {{ config('bank-account')[$order->bank]['number'] }}<br>
									Dari : {{ $order->sender }}
								</td>
								<td>
